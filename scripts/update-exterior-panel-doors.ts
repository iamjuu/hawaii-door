...
async function updateExteriorPanelDoors() {
  try {
    console.log("🚀 Starting 1-to-1 image update for Exterior Panel Doors...\n");
    ...
    // Path to the Exterior Panel Doors folder
    const exteriorPanelFolder = "C:\\Users\\nithi\\Downloads\\EXTERIOR PANEL DOOR";

    // Check if folder exists
    if (!fs.existsSync(exteriorPanelFolder)) {
      console.error("❌ Error: Exterior Panel Doors folder not found at:", exteriorPanelFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(exteriorPanelFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(exteriorPanelFolder, file);
      
      // Extract door name from filename
      // Example: "SD_7282-FP-EPD-Fir.jpg" → "SD 7282 FP EPD Fir"
      const doorName = file
    ...
    console.log(`\n🎉 ALL DOOR TYPES COMPLETE! Your catalog is fully updated!`);
    console.log(`==========================================\n`);
