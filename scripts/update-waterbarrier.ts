...
async function updateWaterbarrier() {
  try {
    console.log("🚀 Starting 1-to-1 image update for Waterbarrier...\n");
    ...
    // Path to the Waterbarrier folder
    const waterbarrierFolder = "C:\\Users\\nithi\\Downloads\\water barrier";

    // Check if folder exists
    if (!fs.existsSync(waterbarrierFolder)) {
      console.error("❌ Error: Waterbarrier folder not found at:", waterbarrierFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(waterbarrierFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(waterbarrierFolder, file);
      
      // Extract door name from filename
      // Example: "SD_37015-FD-WB-Fir_Comp.jpg" → "SD 37015 FD WB Fir Comp"
      const doorName = file
