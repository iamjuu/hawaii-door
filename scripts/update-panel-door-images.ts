...
async function updatePanelDoorImages() {
  try {
    console.log("🚀 Starting bulk image update for Interior Panel Doors...\n");
    ...
    // Path to the panel doors folder
    const panelDoorsFolder = "C:\\Users\\nithi\\Downloads\\panel doors";

    // Check if folder exists
    if (!fs.existsSync(panelDoorsFolder)) {
      console.error("❌ Error: Panel doors folder not found at:", panelDoorsFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(panelDoorsFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(panelDoorsFolder, file);
    ...
    // Find all Interior Panel Doors
    const interiorPanelDoors = await Door.find({
      category: "interior",
      doorType: "Interior Panel Doors",
    }).lean();

    console.log(`🔍 Found ${interiorPanelDoors.length} Interior Panel Doors in database\n`);

    if (interiorPanelDoors.length === 0) {
      console.log("⚠️  No Interior Panel Doors found in database");
      console.log("   Creating sample doors or checking doorType values...\n");
      ...
      console.log("   Available door types:", Array.from(doorTypes));
    ...
    let updatedCount = 0;
    for (const door of interiorPanelDoors) {
      try {
