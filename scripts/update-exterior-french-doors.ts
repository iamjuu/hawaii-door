...
async function updateExteriorFrenchDoors() {
  try {
    console.log("🚀 Starting 1-to-1 image update for Exterior French Doors...\n");
    ...
    // Path to the Exterior French Doors folder
    const frenchDoorsFolder = "C:\\Users\\nithi\\Downloads\\exterior french doors";

    // Check if folder exists
    if (!fs.existsSync(frenchDoorsFolder)) {
      console.error("❌ Error: Exterior French Doors folder not found at:", frenchDoorsFolder);
      process.exit(1);
    }
    ...
    for (const file of imageFiles) {
      ...
      // Example: "SD_7599-FD-Fir.jpg" → "SD 7599 FD Fir"
      const doorName = file
        ...
        .replace(/\s*SL.*$/, '') // Remove "SL-43x250" type suffixes
        .trim();
      ...
      try {
        ...
        const door = await Door.findOne({
          category: "exterior",
          doorType: "Exterior French Doors",
          name: doorName
        });
