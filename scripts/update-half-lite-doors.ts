...
async function updateHalfLiteDoors() {
  try {
    console.log("🚀 Starting 1-to-1 image update for Half Lite Doors...\n");
    ...
    // Path to the Half Lite Doors folder
    const halfLiteFolder = "C:\\Users\\nithi\\Downloads\\half litedoors";

    // Check if folder exists
    if (!fs.existsSync(halfLiteFolder)) {
      console.error("❌ Error: Half Lite Doors folder not found at:", halfLiteFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(halfLiteFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(halfLiteFolder, file);
      
      // Extract door name from filename
      // Example: "SD_37944-HLD-Fir.jpg" → "SD 37944 HLD Fir"
      const doorName = file
