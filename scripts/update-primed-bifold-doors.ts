...
async function updatePrimedBifoldDoors() {
  try {
    console.log("🚀 Starting bulk image update for Primed Bifold Doors...\n");
    ...
    // Path to the primed bifold doors folder
    const primedBifoldFolder = "C:\\Users\\nithi\\Downloads\\primed bifold";

    // Check if folder exists
    if (!fs.existsSync(primedBifoldFolder)) {
      console.error("❌ Error: Primed Bifold Doors folder not found at:", primedBifoldFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(primedBifoldFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(primedBifoldFolder, file);
    ...
    // Find all Primed Bifold Doors
    const primedBifoldDoors = await Door.find({
      category: "interior",
      doorType: "Primed Bifold Doors",
    }).lean();

    console.log(`🔍 Found ${primedBifoldDoors.length} Primed Bifold Doors in database\n`);

    if (primedBifoldDoors.length === 0) {
      console.log("⚠️  No Primed Bifold Doors found in database");
    ...
    let updatedCount = 0;
    for (const door of primedBifoldDoors) {
      try {
