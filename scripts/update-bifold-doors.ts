...
async function updateBifoldDoors() {
  try {
    console.log("🚀 Starting bulk image update for Bifold Doors...\n");
    ...
    // Path to the bifold doors folder
    const bifoldFolder = "C:\\Users\\nithi\\Downloads\\bifold";

    // Check if folder exists
    if (!fs.existsSync(bifoldFolder)) {
      console.error("❌ Error: Bifold Doors folder not found at:", bifoldFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(bifoldFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(bifoldFolder, file);
    ...
    // Find all Bifold Doors
    const bifoldDoors = await Door.find({
      category: "interior",
      doorType: "Bifold Doors",
    }).lean();

    console.log(`🔍 Found ${bifoldDoors.length} Bifold Doors in database\n`);

    if (bifoldDoors.length === 0) {
      console.log("⚠️  No Bifold Doors found in database");
