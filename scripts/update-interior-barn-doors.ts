...
async function updateInteriorBarnDoors() {
  try {
    console.log("🚀 Starting bulk image update for Interior Barn Doors...\n");
    ...
    // Path to the interior barn doors folder
    const barnDoorsFolder = "C:\\Users\\nithi\\Downloads\\interior barn doors";

    // Check if folder exists
    if (!fs.existsSync(barnDoorsFolder)) {
      console.error("❌ Error: Interior Barn Doors folder not found at:", barnDoorsFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(barnDoorsFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(barnDoorsFolder, file);
    ...
    // Find all Interior Barn Doors
    const interiorBarnDoors = await Door.find({
      category: "interior",
      doorType: "Interior Barn Doors",
    }).lean();

    console.log(`🔍 Found ${interiorBarnDoors.length} Interior Barn Doors in database\n`);

    if (interiorBarnDoors.length === 0) {
      console.log("⚠️  No Interior Barn Doors found in database");
    ...
    let updatedCount = 0;
    for (const door of interiorBarnDoors) {
      try {
