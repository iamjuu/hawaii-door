...
async function updateInteriorFrenchDoors() {
  try {
    console.log("🚀 Starting bulk image update for Interior French Doors...\n");
    ...
    // Path to the interior french doors folder
    const frenchDoorsFolder = "C:\\Users\\nithi\\Downloads\\interior french doors";

    // Check if folder exists
    if (!fs.existsSync(frenchDoorsFolder)) {
      console.error("❌ Error: Interior French Doors folder not found at:", frenchDoorsFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(frenchDoorsFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(frenchDoorsFolder, file);
    ...
    // Find all Interior French Doors
    const interiorFrenchDoors = await Door.find({
      category: "interior",
      doorType: "Interior French Doors",
    }).lean();

    console.log(`🔍 Found ${interiorFrenchDoors.length} Interior French Doors in database\n`);

    if (interiorFrenchDoors.length === 0) {
      console.log("⚠️  No Interior French Doors found in database");
    ...
    let updatedCount = 0;
    for (const door of interiorFrenchDoors) {
      try {
