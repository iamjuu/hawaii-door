...
async function updateCraftsmanCollection() {
  try {
    console.log("🚀 Starting 1-to-1 image update for Craftsman Collection...\n");
    ...
    // Path to the Craftsman Collection folder
    const craftsmanFolder = "C:\\Users\\nithi\\Downloads\\Craftssman collection";

    // Check if folder exists
    if (!fs.existsSync(craftsmanFolder)) {
      console.error("❌ Error: Craftsman Collection folder not found at:", craftsmanFolder);
      process.exit(1);
    }

    // Read all image files from the folder
    const files = fs.readdirSync(craftsmanFolder);
    ...
    for (const file of imageFiles) {
      const filePath = path.join(craftsmanFolder, file);
      
      // Extract door name from filename
      // Example: "SD_36933-Palomar-Fir.jpg" → "SD 36933 Palomar Fir"
      const doorName = file
        ...
        .replace(/\s*\(\d+\)\s*$/, '') // Remove "(1)" type suffixes
        .trim();
